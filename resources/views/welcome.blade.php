...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-[#FDFDFC] dark:bg-[#0a0a0a] text-[#1b1b18] flex p-6 lg:p-8 items-center min-h-screen flex-col">
        ...
        <header class="w-full lg:max-w-4xl text-sm mb-6 flex justify-end gap-4">
            @if (Route::has('login'))
                @auth
                    <a href="{{ url('/dashboard') }}" class="px-5 py-1.5 border border-[#19140035] dark:text-[#EDEDEC] dark:border-[#3E3E3A] rounded-sm">Dashboard</a>
                @else
                    <a href="{{ route('login') }}" class="px-5 py-1.5 dark:text-[#EDEDEC]">Log in</a>
                    @if (Route::has('register'))
                        <a href="{{ route('register') }}" class="px-5 py-1.5 border border-[#19140035] dark:text-[#EDEDEC] dark:border-[#3E3E3A] rounded-sm">Register</a>
                    @endif
                @endauth
            @endif
        </header>
            ...
            <main class="w-full lg:max-w-5xl">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    @forelse($donations ?? [] as $donation)
                        <div class="flex flex-col lg:flex-row bg-white dark:bg-[#161615] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-lg overflow-hidden">
                            ...
                            <div class="p-6 flex flex-col justify-between">
                                <div>
                                    <h2 class="font-medium text-lg mb-2">{{ $donation->title }}</h2>
                                ...
                                <div class="mt-4 pt-4 border-t border-[#e3e3e0] dark:border-[#3E3E3A]">
                                    <p class="text-[13px] font-bold text-[#f53003]">
                                        Target: Rp {{ number_format($donation->amount, 0, ',', '.') }}
                                    </p>
                                    <p class="text-[11px] text-[#706f6c]">{{ $donation->bank_info }}</p>
                                </div>
                            </div>

                        </div>
                    @empty
                        <div class="col-span-full p-12 text-center border-2 border-dashed border-[#e3e3e0] rounded-lg">
                            <p class="text-[#706f6c]">Belum ada data donasi.</p>
                        </div>
                    @endforelse
                </div>
            </main>
        ...
        <footer class="py-12 text-[13px] text-[#706f6c]">
